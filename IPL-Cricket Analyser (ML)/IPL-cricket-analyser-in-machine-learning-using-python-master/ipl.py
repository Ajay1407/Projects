# -*- coding: utf-8 -*-
"""
Created on Sat Nov  4 18:52:49 2017

@author: RANJEEV NAYAK
"""
...
from tkinter import *
root = Tk()
ButtonImage = PhotoImage(file='cick.png')

#f=Frame(root,image=ButtonImage)
back=Frame(master=root, width=976, height=631)
back.pack_propagate(0)
back.pack(fill=tk.BOTH, expand=1)
testButton = Button(back, image=ButtonImage)
testButton.pack()
matches.head(2)
...
def dreamteam():  
        ...
        print("BEST IPL TEAM ");
        print(ax+ax1)
butt=Button(root,text="IPL INFO",command=dreamteam,padx=5,pady=10)
butt.pack(side=LEFT) 
        
def datainfo():
            tossdecision()
button2=Button(root,text="TOSS DECISION",command=datainfo,padx=5,pady=10)
button2.pack(side=LEFT) 
        
def genre():
            maxtosswin()
            
a=Button(root,text=" MAX TOSS WINNER",command= genre, justify=LEFT,padx=5,pady=10)
a.pack(side=LEFT)
    
def pred():
            tosswinneralsomatchwinner()
c=Button(root,text=" IS TOSS WINNER ALSO A MATCH WINNER ",command= pred,padx=5,pady=10)
c.pack(side=LEFT) 
        
def analys1():
            runsacrosseachseason()
b1=Button(root,text="RUN ON EACH SESSION",command= analys1,padx=5,pady=10)
b1.pack(side=LEFT)
    
def genre1():
            averagerunspermatch()
a1=Button(root,text=" AVERAGE PER MATCH ",command= genre1, justify=LEFT,padx=5,pady=10)
a1.pack(side=LEFT)    
        
def ana():
            favouritegrounds()
e=Button(root,text="FAVORITE GROUNDS",command= ana,padx=5,pady=10)
e.pack(side=LEFT)   
        
def play():
            maximummanofthematches()
g=Button(root,text=" MAX MAN OF MATCH ",command= play,padx=5,pady=10)
g.pack(side=LEFT)  
...
h=Button(root,text="PLAYER COMPARISION",command= goal,padx=5,pady=10)
h.pack(side=LEFT)  
 
def team():
            dreamteam()
o=Button(root,text="DREAMTEAM",command= team,padx=5,pady=10)
o.pack(side=LEFT)
        
root.title("ipl viewers")
root.mainloop() 
